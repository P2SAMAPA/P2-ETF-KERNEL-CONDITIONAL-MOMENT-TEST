...
st.set_page_config(page_title="Kernel Conditional Moment Test for Predictability", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Kernel Conditional Moment Test for Predictability</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Kernel‑based test for non‑linear predictability from macro variables</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Kernel Test")
...
st.sidebar.markdown(f"**Kernel bandwidth σ:** {config.KERNEL_BANDWIDTH}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'kernel_conditional_moment_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Predictability score: {etf['kcm_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Predictability"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Predictability", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Kernel Conditional Moment Predictability (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Kernel conditional moment test** detects non‑linear predictability from macro variables.
        - First, we regress ETF returns on macro variables (linear model) to remove linear dependence.
        - The test statistic T = (1/n²) Σ_{i,j} K(macro_i, macro_j) * resid_i * resid_j.
        - Higher T → macro variables have non‑linear predictive power beyond linear models.
        - High score → ETF returns are predictable from macro (potential alpha source).
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the predictability scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Kernel Conditional Moment Test | Non‑linear predictability from macro")
