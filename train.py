...
from kernel_conditional_moment import kernel_conditional_moment_test
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        s = kernel_conditional_moment_test(
    ...
    top_etfs = [{"ticker": t, "kcm_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/kernel_conditional_moment_{timestamp}.json"
